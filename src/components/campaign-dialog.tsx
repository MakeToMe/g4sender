...
import { createCampaign, updateCampaign, getPresignedUrl, Campaign } from "@/app/actions/campaigns"
...
export function CampaignDialog({ children, campaign, open: controlledOpen, onOpenChange }: CampaignDialogProps) {
    ...
    const [mediaPreview, setMediaPreview] = useState<string | null>(null) // We can't easily preview existing URL as Blob without CORS sometimes, but let's try to show existing URL if needed or just filename
    const [isPending, startTransition] = useTransition()
    const [isUploading, setIsUploading] = useState(false)

    // Reset form when opening for create, or load when editing
    useEffect(() => {
        if (open) {
            setName(campaign?.name || "")
            setMessage(campaign?.message_template || "")
            setMessageType(campaign?.message_type || "text")
            //@ts-ignore - 'message_url' might not be in the type definition if not updated yet, but assuming it comes from DB
    ...
    const handleSubmit = async () => {
        ...
        startTransition(async () => {
            ...
            } else if (campaign && (campaign as any).message_url && messageType !== 'text') {
                // Keep existing URL if not changed
                formData.append('message_url', (campaign as any).message_url)
            }
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[900px]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-4">
                        ...
                        {messageType !== 'text' && (
                            <div className="grid gap-2">
                                ...
                                {mediaPreview && !mediaFile && (
                                    <p className="text-xs text-green-600 truncate">Mídia atual: ...{mediaPreview.slice(-15)}</p>
                                )}
                            </div>
                        )}
                    ...
                    <div className="flex justify-center items-center bg-slate-50 rounded-lg p-4 border">
                        {/* Phone Mockup */}
